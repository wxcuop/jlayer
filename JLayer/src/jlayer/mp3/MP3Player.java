...
public class MP3Player{
	...
	private final int lostFrames = 20; //some fraction of a second of the sound gets "lost" after every pause
	...
	public boolean play(int frameIndexStart, int frameIndexFinal, int correctionFactorInFrames) throws JavaLayerException{
		...
		if (this.listener != null) {
			this.listener.playbackStarted(new PlaybackEvent(this, EventType.Started, this.audioDevice.getPosition()));
		}
		...
		if (this.audioDevice != null && !this.paused){
			...
			if (this.listener != null) {
				...
				PlaybackEvent playbackEvent = new PlaybackEvent(this, EventType.Stopped, audioDevicePosition);
	...
	public void pause(){
		if(!this.stopped){
			this.paused = true;
			if (this.listener != null) {
				this.listener.playbackPaused(new PlaybackEvent(this, EventType.Paused, this.audioDevice.getPosition()));
			}
			this.close();
		}
	}
	...
	public void stop(){
		if(!this.stopped){
			if(!this.closed){
				this.listener.playbackFinished(new PlaybackEvent(this, EventType.Stopped, this.audioDevice.getPosition()));
				this.close();
			}
			else if(this.paused){
				int audioDevicePosition = -1; //this.audioDevice.getPosition(), audioDevice is null
				this.listener.playbackFinished(new PlaybackEvent(this, EventType.Stopped, audioDevicePosition));
			}
			this.stopped = true;
		}
	}
